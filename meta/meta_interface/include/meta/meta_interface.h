...
namespace sam::Meta::Key
{
  ...
  //                         KeyMeta Template                         //
  //------------------------------------------------------------------//
  template <typename DerivedKeyMeta,
            const char* NAME,
            typename KEY_T,
            typename TANGENT_SPACE_T = KEY_T,
  ...
  struct Base
  {
    ...
    static constexpr const std::array<const char*, kNbComponents> components {
        ORDERRED_COMPONENT_NAMEs...};

    using key_t = KEY_T;   // might be a vector, SE2 SO3 etc.. // TODO: use Key_t rather than key_t
    using type  = KEY_T;
    ...
    // using type = KeyMeta<DerivedKeyMeta, NAME, KEY_T,TANGENT_SPACE_T,
    // ORDERRED_COMPONENT_NAMEs...>;
...
namespace sam::Meta::Measure
{
  ...
  struct Base
  {
    ...
    // measure type : is often a vector, but could be also manifold such SE(n) type (e.g. from scan
    // matcher in pose graphs problems)
    ...
    // Get a subset of themeasure by component (e.g. 'y' of a absolute position measurement, or 't'
    // of a SE2 measure) This necessarily defers to the derived class
    template <const char* COMPONENT>
    static auto get_component(const measure_t& measure)
    {
      static_assert(((*COMPONENT == *ORDERRED_COMPONENT_NAMEs) || ...)
                    //, std::format("a")
      );
      // return static_cast<DerivedMeasureMeta*>(this)->template
      // get_component_impl<COMPONENT>(measure);
