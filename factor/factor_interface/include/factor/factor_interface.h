...
namespace details_sam::Conduct
{
   ...
   */
  template <typename DerivedKCC, typename KEY_META, typename MEASURE_META, const char* ContextRole>
  struct KeyContextualConduct : KEY_META
  {
     ...
     * \partial X_k Index i refers to the usual iterator on factor, k to the kth key of factor
     * \phi_i
     *
     * @param Xk linearization point
     * @return the process matrix associated with this key. Note that it only a subset of the
     * columns of the full process matrix of factor \phi_i as there may be other keys k in the
     * factor with their own sub process-matrix Hik.
     */
  ...
  /**
   * @brief KeyContextualConduct  -- The class describes how a key (associated with a key meta)
   * behaves in a certain context (in a linear factor). It stores dimension informations name and
   * most importantly process matrices to be used when assessing the measure expectation wrt the
   * decision keys. As a factor can have several keys, therefore, the LinearKeyContextualConduct
   * class only manages a subset of the columns of the factor 's process matrix.
   *
            ...
            ContextRole>
  {
    ...
    static_assert(std::is_same_v<std::remove_pointer_t<decltype(Hik_MATRIX_PTR)>,
    ...
    // inherite ctor
    LinearKeyContextualConduct(const std::string& key_id) : base_kcc_t(key_id) {}
...
namespace sam::Factor
{
   ...
   * - the model that links the measurement it to its set of hidden variables \cal X := { X_k },
   * also called keys.
   *
   ...
   * - the model r(X;Z) varies factor to factor, which in turns implies that the Jacobian
   * derivatives of this function wrt X do vary too.
   *
   ...
   */
  template <typename DerivedFactor, const char* FactorLabel, typename KCC, typename... KCCs>
  class BaseFactor
  {
    public:
    ...
    using composite_of_opt_state_ptr_t
        = std::tuple<std::optional<std::shared_ptr<typename KCC::Key_t>>,
                     std::optional<std::shared_ptr<typename KCCs::Key_t>>...>;
    //  NOTE: Xk same type as dXk in euclidian factors
    using matrix_Ai_t = Eigen::Matrix<double, kM, kN>;
    using matrices_Aik_t
        = std::tuple<typename KCC::key_process_matrix_t, typename KCCs::key_process_matrix_t...>;
    using KeysSet_t = std::tuple<KCC, KCCs...>;
    using KeyMetas_t
        = std::tuple<typename KCC::KeyMeta_t, typename KCCs::KeyMeta_t...>;   // not unique
    ...
    static_assert(
        (std::is_same_v<typename KCC::MeasureMeta_t, typename KCCs::MeasureMeta_t> && ...));
     ...
     * some key points is missing (e.g. observation of a new landmark) For every combination of
     ...
     *
     * Since init points for all keys are necessary for the factor construction, failure to guess
     * one or more missing initial key point prevents the factor instantiation. It is the
     * responsibility of system designer to come up with adequate recovery approaches at higher
     * level (e.g. waiting for several bearing-only observations to define triangulation scheme).
     *
     * If all initial values are available anyway, this method does nothing
     *
     * @param x_init_ptr_optional_tup tuple containing, for each key in the factor type, the
     * optional pointers to values of initial key points value. If no initial value is available in
     * inputs, std::nullopt must be passed.
    ...
    static composite_state_ptr_t make_composite(typename KCC::Key_t key,
                                                typename KCCs::Key_t... keys)
    {
      return std::make_tuple(std::make_shared<typename KCC::Key_t>(key),
                             std::make_shared<typename KCCs::Key_t>(keys)...);
    }
    ...
    {
      return std::apply([](const auto&... key_set)
    ...
    // /**
    //  * @brief returns square root of the negative log likelihood at the current linearization
    //  point
    //  * \bar X.
    //  *  - log ( p(Z|X) ) = r(X;Z)^T r(X;Z) = || r(X;Z) ||^2_2 = norm ^2
    //  *
    ...
    static KeysSet_t construct_keys_set(const std::array<std::string, kNbKeys>& keys_id)
    {
      if constexpr (kNbKeys > 1)
      {
        ...
        for (int i = 1; i < kNbKeys; i++) { sub_keys_id[i - 1] = keys_id[i]; }
      ...
      else { return std::make_tuple(KCC(keys_id[0])); }
    }
   ...
   * also called keys.
   *
   * The Euclidian Factor template 'statically' derives from the Base Factor. It cover the cases
   * where r(X) is of the form: r(X) = \rho ( h(X) - z ) I.e. the measure z and h(X) are both
   * euclidian vectors The keys X might still not be expressed in euclidian space though (e.g.
   * landmark cartesian observation). Look at LinearEuclidianFactor for a class template that
   * supports euclidian keys.
   *
   * A factor is a representation of the pdf likelihood p(Z = z | \cal X) .
   * It is assumed that p( Z = z | \mathcal X ) \propto \exp^ { - r(X;Z)^T r(X;Z) }
   * This block is class template because:
  ...
  class EuclidianFactor
      : public BaseFactor<EuclidianFactor<DerivedEuclidianFactor, FactorLabel, KCC, KCCs...>,
                          FactorLabel,
                          KCC,
                          KCCs...>
  {
    ...
    public:
    using BaseFactor_t
        = BaseFactor<EuclidianFactor<DerivedEuclidianFactor, FactorLabel, KCC, KCCs...>,
                     FactorLabel,
                     KCC,
                     KCCs...>;
     ...
     * @return nested tuple of {bi , tuple (Aiks...) }
     */
    std::tuple<criterion_t, matrices_Aik_t>
        compute_Ai_bi_at_impl(const composite_state_ptr_t& X) const
    {
      criterion_t bi = -this->compute_r_of_x_at(X);
      // matrices_Aik_t all_Aik ;
      // double apply pattern to zip two tuples
      matrices_Aik_t all_Aik = std::apply(
          [this, &X](const auto&... kcc)
          {
            return std::apply(
                [&](const auto&... Xkptr)
                          ...
                          LinearKCCs...>
  {
    ...
    using BaseFactor_t = BaseFactor<
        LinearEuclidianFactor<DerivedLinearEuclidianFactor, FactorLabel, LinearKCC, LinearKCCs...>,
        FactorLabel,
        LinearKCC,
        LinearKCCs...>;
    // declares friendlies so that they get to protected impl methods of this class
    ...
    LinearEuclidianFactor(const std::string&                                    factor_id,
        ...
        , all_Aik(std::apply(
              [this](const auto&... kcc)
              {
                // HACK: it seems I must cast otherwise I get Eigen::Product< T1, T2> whichh doesn't
                // cast automatically (somehow) into Eigen< T3> where T3 is the matrix type of T1*T2
                // return std::make_tuple(
                // std::decay_t<std::remove_const_t<decltype(kcc.Hik)>>(this->rho * kcc.Hik) ... );
                return std::make_tuple(
                    typename std::decay_t<std::remove_const_t<decltype(kcc)>>::key_process_matrix_t(
                        this->rho * kcc.Hik)...);
              },
              this->keys_set))
    ...
    criterion_t compute_h_of_x(const composite_state_ptr_t& X) const
    {
      ...
      return std::apply([&](const auto&... HkXk) { return (HkXk + ...); }, HkXks);
    }
