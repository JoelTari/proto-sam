...
namespace sam::Marginal
{
  ...
  struct WrapperPersistentMarginal
  {
     ...
     WrapperPersistentMarginal<MARGINAL_T> & operator=(const WrapperPersistentMarginal<MARGINAL_T> & other)
     {
  ...
  class MarginalsContainer
  {
    ...
    // find a mean ptr, the same as above, but more practical to build composite state of optionals (when guessing incomplete init point)
    template <typename Q_KEYMETA_T>
    ...
    static constexpr std::size_t get_correct_tuple_idx()
    {
      ...
      if constexpr (std::is_same_v<typename std::tuple_element_t<TUPLE_IDX, Marginals_Data_t>::mapped_type::Marginal_t::KeyMeta_t,
                                   Q_KEYMETA_T>)
      { return TUPLE_IDX; }
      else
      {
    ...
    static constexpr std::size_t get_correct_tuple_idx_by_wmarg()
    {
      ...
      {
        return get_correct_tuple_idx_by_wmarg<Q_WMARG_T,TUPLE_IDX + 1>();
      }
    }
  ...
  // visual covariance 
  // WARNING: it is assumed, for now, that the first 2x2 block of the cov matrix hold x,y marginal covariance
