...
dataRate=rospy.get_param('/front_end_node/dataRate',50)
...
tag_detection_threshold=rospy.get_param('/front_end_node/tag_detection_threshold',15)
        ...
        while True:
            ...
            if tags_detected:
                ...
                if tags.header.stamp < tf1.stamp:
                        ...
                        if abs(np.linalg.det(R1)) > 0 or  abs(np.linalg.det(R2)) > 0:
                            ...
                            rospy.loginfo("[INTERPOLATION] Singular matrix of covariance")
...
def transform_tags(tf,output):
    ...
    for detection in tags.detections:
        ...
        if tag.r < tag_detection_threshold:
            if  out_pose.position.x<1e-3:
                tag.phi=0
            else:
                tag.phi=m.atan2(out_pose.position.y,out_pose.position.x)
            tag.covariance=[0.0016*(tag.r)**2,0,0,
                            0,0.0123*(tag.phi)**2,0,
                            0,0,1]  
            out_tags.append(tag)
...
def transform_odom(tf1,output):
    ...
    output.E.v=tf1.v 
    output.E.w=tf1.w 
    output.E.covariance=[tf1.covariance[0],tf1.covariance[1],0,
                         tf1.covariance[2],tf1.covariance[3],0,
                         0,0,0.08] #1 degree standard deviation on final pose
...
def front_end_node():
        ...
        pub = rospy.Publisher('/output_front_end', EdgesStamped, queue_size=15)
