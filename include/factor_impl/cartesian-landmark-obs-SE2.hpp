...
namespace details_sam::Factor{
  namespace CartesianObsSE2Impl{
        ...
        <SightedLandmarkKeyConduct,Key::Position2d,Measure::LinearTranslation2d,sighted_role_str>
    {
      ...
      // the ctors
      SightedLandmarkKeyConduct(const std::string& key_id, const measure_cov_t& rho) : BaseKeyCC_t(key_id, rho)
      {}
      SightedLandmarkKeyConduct(const std::string& key_id, const measure_cov_t& rho, std::shared_ptr<Key_t> init_point_view) : BaseKeyCC_t(key_id, rho, init_point_view)
      {}
    };
