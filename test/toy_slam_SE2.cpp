...
class ToySLAMSE2System : public ::testing::Test
{
  ...
  void SetUp() override
  {
    ...
    // t0
    PreRegistrationBundleCollection<AnchorSE2, MotionModelSE2, LandmarkCartesianObsSE2>::tuple_t
        collection_t0;
    // f0
    {
    ...
    // f1
    {
      ...
      std::get<2>(collection_t0).push("f1", z, cov_landmark, {"b0", "x0"});
    }
    // f2
    {
      auto z = ::sam::Measure::Motion2d_t(3.0034, -5.2488);
      std::get<2>(collection_t0).push("f2", z, cov_landmark, {"b1", "x0"});
    }
    // f3
    {
      auto z = ::sam::Measure::Motion2d_t(7.3764, -2.6761);
      std::get<2>(collection_t0).push("f3", z, cov_landmark, {"b3", "x0"});
    }
  ...
  void process_tests(SYS_T& sys)
  {
    for (const auto& collection : this->collections.vector_of_collection)
    {
      ...
      if (AnchorBundle.size() > 0)
        sys.template register_factors_in_bundle<AnchorSE2>(AnchorBundle.vfid,
                                                           AnchorBundle.vmeasure,
                                                           AnchorBundle.vmeasure_cov,
                                                           AnchorBundle.vkeys_id);
      if (!MotionModel.empty())
        sys.template register_factors_in_bundle<MotionModelSE2>(MotionModel.vfid,
                                                                MotionModel.vmeasure,
                                                                MotionModel.vmeasure_cov,
                                                                MotionModel.vkeys_id);
      if (!LandmarkObs.empty())
        sys.template register_factors_in_bundle<LandmarkCartesianObsSE2>(LandmarkObs.vfid,
                                                                         LandmarkObs.vmeasure,
                                                                         LandmarkObs.vmeasure_cov,
                                                                         LandmarkObs.vkeys_id);
    }
